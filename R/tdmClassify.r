...
tdmClassify <- function(d_train,d_test,d_dis,response.variables,input.variables,opts)
{
    ...
    for (response.variable in response.variables) {   
        ...
        if (!is.null(opts$CLASSWT)) names(opts$CLASSWT) <- lev.resp;
        opts$RF.cutoff <- tdmModAdjustCutoff(opts$RF.cutoff,n.class);
        opts$SRF.cutoff <- tdmModAdjustCutoff(opts$SRF.cutoff,n.class);
  
        if (is.null(opts$gainmat)) {
          # default (generic) gain matrix:
          opts$gainmat <- matrix ( diag(n.class),nrow=n.class,ncol=n.class,
                                  dimnames=list(lev.resp,   # row names    (true levels)
                                                lev.resp    # column names (predicted levels)
                                                ))
          # instead of diag(n.class) an existing opts$gainmat can specify a user def'd gain matrix, e.g. for n.class=2:
        ...
        if (!is.null(opts$MOD.SEED)) set.seed(newseed) # if you want reproducably the same model training,
        ...
        if (is.null(opts$MOD.SEED)) set.seed(tdmRandomSeed());                                                                  
        ...
        train.rf <- function(response.variable,to.model,opts) {
            ...
            if (!is.null(opts$CLASSWT)) {
                ...
                cwt = opts$CLASSWT*1.0;   # strange, but necessary: if we omit '*1' then cwt seems to be a copy-by-reference of
                                          # opts$CLASSWT. After randomForest call  cwt is changed and also opts$CLASSWT would be changed (!)
            } else { cwt=NULL; }
            if (!is.null(opts$RF.cutoff)) cat1(opts,"Cutoff: ", opts$RF.cutoff,"\n")
            ...
            # the 'non-speaking' variables cwt, opts$RF.cutoff and add below res.rf$cutoff, res.rf$classwt 
            ...
            res.rf$cutoff = opts$RF.cutoff;
            res.rf$classwt = opts$CLASSWT;
            res.rf;
        } 
        ...
        train.svm <- function(response.variable,to.model,opts) {
            cat1(opts,filename,": Train SVM ...\n")
	          require(e1071)			
            if (!is.null(opts$CLASSWT)) cat1(opts,"Class weights: ", opts$CLASSWT,"\n")
            flush.console();
            formul <- formula(paste(response.variable, "~ ."))   # use all possible input variables
            res.rf <- svm(formul 
            							, data=to.model 
        ...
        if (res.rf[1]=="INVALID") {
          cat1(opts,sprintf("*** Invalid opts$method=%s ***\n",opts$method));
        }
        #print(res.rf)
        ...
        apply.rf <- function(res.rf,to.model,to.test,to.dis,opts) {        
            cat1(opts,filename,": Apply",opts$method,"...\n")
            app = list()            
        ...
        apply.SVM <- function(res.rf,to.model,to.test,to.dis,opts) {
            ...
            if (is.null(opts$RF.cutoff)) {
              app$train.predict <- predict(res.rf, newdata=to.model)
            } else {
              cat1(opts,"Cutoff: ", opts$RF.cutoff,"\n")
  			      # Apply new weighting scheme for training set            
     			    app$train.predict <- apply(app$train.prob/(matrix(1,nrow(to.model),1) %*% opts$RF.cutoff),1,which.max);
              ...
              app$test.predict <- predict(res.rf, newdata=to.test)
        			...
        			# Apply new weighting scheme for test set            
     			    app$test.predict <- apply(app$test.prob/(matrix(1,nrow(to.test),1) %*% opts$RF.cutoff),1,which.max)
              ...
              if (is.null(opts$RF.cutoff)) {
                app$dis.predict <- predict(res.rf, newdata=to.dis)
          			...
          			# Apply new weighting scheme for disregard set (unknown label data in active learning)
       			    app$dis.predict <- apply(app$dis.prob/(matrix(1,nrow(to.dis),1) %*% opts$RF.cutoff),1,which.max)
        ...
        apply.CMB <- function(to.model,to.test,to.dis,opts) {
            ...
            app$train.prob <- opts$result1$last_res$last.probs$v_train * opts$result2$last_res$last.probs$v_train;
      			app$test.prob <- opts$result1$last_res$last.probs$v_test * opts$result2$last_res$last.probs$v_test;
      			app$dis.prob <- opts$result1$last_res$last.probs$v_dis * opts$result2$last_res$last.probs$v_dis;
        ...
        if (res.rf$HasVotes) {  # column "votes" is the fraction of trees voting for 1st class
          name.of.votes <- "votes";
        ...
        if (res.rf$HasProbs) {
          last.probs = NULL;                                        # last.probs is used by ssl_methods.r
          if (opts$method %in% c("RF","MC.RF")) {
            last.probs = list(  v_train = res.rf$votes                
                              , v_test = app$test.votes
                              , v_dis = app$dis.votes
                              );
          }
          if (opts$method %in% c("SVM","CMB")) {
            last.probs = list(  v_train = app$train.prob             
    ...
    ######################################################################################
        ...
        if (opts$GRAPHDEV!="non" & length(input.variables)>1) {
        ...
        if (opts$GRAPHDEV!="non") {
            ...
            tdmGraphicNewWin(opts);
            cmt<-cm.test$mat
            heigth<-t(matrix(c(diag(cmt),rowSums(cmt)-diag(cmt)),nrow=n.class,ncol=2))
    ...
    res =   list(last.cm.train=cm.train   # from last response.variable 
                ,last.cm.test=cm.test     #   "     "     "       "
                ,last.rf = res.rf         #   "     "     "       "
                ,last.pred = name.of.prediction #   "     "       "
                ,last.probs = last.probs  # probability matrix (row:records, col: classes) or NULL
                ,sumEVAL=sumEVAL
                ,allEVAL=allEVAL
                ,d_train=d_train
                ,d_test=d_test 
...
tdmClassifyLoop <- function(dset,response.variables,input.variables,opts) {
    ...
    for (i in 1:opts$NRUN) {  
        ...
        if (!is.null(opts$TST.SEED)) set.seed(newseed);  # if you want reproducably the same training/test sets,
        ...
        G_test2[i] = Err["gain.tst2"]             # for comparision in case of opts$method="RF": results with
        R_test2[i] = Err["rgain.tst2"]            # default cutoff 1/n.class instead of opts$RF.cutoff
    ...
    #write.table(d_test, file=paste(dir.output, sub(".csv", "", filename), "_predictions.csv", sep=""), quote=FALSE, sep=";", dec=".", row.names=FALSE, col.names=TRUE)
    ...
    result = list(opts = res$opts
              	, last_res = res     # last run, last fold: result from tdmClassify
              	, C_train = C_train
              	, G_train = G_train
              	, R_train = R_train
              	, C_test = C_test
    ...
    class(result) <- c("TDMclassifier","TDM")     # this causes > result; or > print(result);
                                                  ...
                                                  # but to call instead the function  printTDMclassifier
...
#'
#' @references   \code{\link{tdmClassify}}, \code{\link{tdmClassifyLoop}}, \code{\link{tdmRegressSummary}}
#' @author Wolfgang Konen, FHK, Sep'2010 - Oct'2011
#' @export
######################################################################################
...
{
    res <- result$last_res;  
    #print2(opts,res$allEVAL);		      # EVAL for each response variable , but only for last_res
    ...
    z=data.frame(TYPE=c("rgain","meanCA","minCA")
                ,DESC=c("relative gain","mean class accuracy","minimum class accuracy"));
    cat1(opts,sprintf("\nRelative gain is '%s' (%s)",opts$rgain.type, z$DESC[which(z$TYPE==opts$rgain.type)]));
    cat1(opts,sprintf("\nTest2 relative gain (predict always with %s): %7.3f", res$allEVAL$test2.string, mean(result$R_test2)));  
    cat1(opts,ifelse(opts$NRUN>1,sprintf(" +-%7.3f",sd(result$R_test2)),""));
    cat1(opts,sprintf("   (on %d records)\n",nrow(res$d_test)));
    if (opts$method %in% c("RF","MC.RF") & opts$RF.OOB==TRUE) {
...
show.cm.train <- function(result) {
  opts = result$opts;
  opts$VERBOSE = 2;
  cm.train <- result$last_res$last.cm.train;
  cat1(opts,"Training cases (",nrow(result$last_res$d_train),"):\n")
  if (opts$VERBOSE>=1) print(cm.train$mat)                     # confusion matrix on training set
...
show.cm.test <- function(result) {
  opts = result$opts;
  opts$VERBOSE = 2;
  cm.test <- result$last_res$last.cm.test;
  n.test <- nrow(result$last_res$d_test);
  if (n.test>0) {
...
predict.TDMclassifier <- function(result,newdata,...) {
  predict(result$last_res$last.rf,newdata,...);
}
