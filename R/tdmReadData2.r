...
#'
#' Read the data accoring to the settings \code{opts$READ.TXT, opts$READ.TrnFn, opts$READ.TstFn, 
#' opts$TST.COL} (see Details).
#'
#Details
#' When \code{opts$READ.TstFn==NULL}, then only \code{opts$READ.TrnFn} is used. \cr 
#' When \code{opts$READ.TstFn!=NULL}, the following things happen in \code{\link{tdmReadData2}}: 
...
tdmReadTrain <- function(opts) {
  dset = read.csv(file=paste(opts$dir.txt, opts$filename, sep=""), nrow=opts$READ.NROW);   # includes header=T, sep="," and dec="."
}

tdmReadTest <- function(opts) {
  dset = read.csv(file=paste(opts$dir.txt, opts$filetest, sep=""), nrow=opts$READ.NROW);   # includes header=T, sep="," and dec="."
}