...
# unbiasedBestRun_C
#
#'     Perform unbiased runs with best-solution parameters (classification).
#'     Read the best solution of a parameter-tuning run (either from envT$bst or from file), 
#'     run with these best parameters the command \code{tdm$mainCommand} (usually a classification machine learning task), to see 
...
#'    envT <- new.env();
#'    tdm <- list(mainFile="main_sonar.r", mainCommand="result <- main_sonar(opts)");
#'    source(tdm$mainFile);
#'    finals <- unbiasedBestRun_C("sonar_04.conf",envT,tdm=tdm)
#'    print(finals);
#'    setwd(oldwd);
#'
#' @references   \code{\link{unbiasedBestRun_R}}
#' @author Wolfgang Konen, FHK, Sep'2010 - Oct'2011
#' @export
#
######################################################################################
unbiasedBestRun_C <- function(confFile,envT,finals=NULL,umode="RSUB",withParams=F,tdm=tdm){
    ...
    writeLines(paste("start unbiased run (classification) for",basename(tdm$mainFile),"..."), con=stderr());
...
#   source(tdm$mainFile)
    ...
    names(add.finals) <-  paste(c("RGain.","sdR."),umode,sep="");
