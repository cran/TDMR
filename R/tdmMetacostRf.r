...
tdmMetacostRf <- function(response.variable,to.model,opts)
{
        ...
        if (DBG) {          # debug info to chase the "Incorrect cutoff specified bug": 
          ...
          if (!is.null(opts$RF.cutoff))
            if (length(levels(to.model[,response.variable]))!=length(opts$RF.cutoff)) {
        ...
        mc.target <- factor(apply(Pjx %*% opts$gainmat, 1,  function(x) { colnames(Pjx)[which.max(x)] } )
                    ...
                    # (Pjx %*% opts$gainmat is a matrix with #rows = #records and the second argument "1" indicates 
        ...
        DBG=F
        if (DBG) {          # debug info to chase the "Incorrect cutoff specified bug": 
          #if (!is.null(opts$RF.cutoff)) cat2(opts,"Cutoff: ", opts$RF.cutoff,"\n")
          print2(opts,c(Targets=NA,table(to.model[,response.variable])))      # debug info: 
          save(formul,to.model,response.variable,rf.options,opts,file=paste(opts$dir.Rdata,"rf_input_dbg.Rdata",sep=""));
          cat1(opts,"RF-debug-data saved to", paste(opts$dir.Rdata,"rf_input_dbg.Rdata",sep=""),"\n");
          if (!is.null(opts$RF.cutoff))
...
applyMetacostRf <- function(app,res.rf,to.test,opts) 
{
        ...
        Pjx = app$test.votes
        mc.target <- as.factor(apply(Pjx %*% opts$gainmat, 1,  function(x) { colnames(Pjx)[which(x==max(x))]}))
